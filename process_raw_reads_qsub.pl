...
-q INT		FASTX Toolkit: min quality score for fastq_quality_trimmer [20]
-p INT		FASTX Toolkit: min percent for fastq_quality_filter [80]
-s INT		FASTX Toolkit: quality score offset type Sanger(33) or Illumina(64) [33]
-bq INT		BWA: quality cutoff
...
-t STR		location to create temp directories [current dir]
-qq STR		qsub -q [batch]
-h 		this message
";
...
#print "\n\n !! Make sure that you run \"sh genome_indexing.sh\" before you process the fastq files !!\n\n";
  print ARRAY_SH "\$ONE=`qsub -q $qsub_q genome_indexing.sh`
echo \$ONE\n";
  push @depend, 'afterok:\$ONE';
}
...
if ($split) {
  ...
  foreach my $sample ( keys %files ) {
    foreach my $file ( @${ $files{$sample} } ) {
      $fq_count++;
      open SPLIT, ">$current_dir/p$fq_count.split_fq.sh"
        or die "Can't open $current_dir/split_fq.sh for writing\n";
      print SPLIT "#!/bin/bash\n\n";
      my $cmd =
"$scripts_dir/fastq_split.pl -s 1000000 -o split_by_number_fq/ $dir_path/$file";
  ...
  open OUTSH, ">$current_dir/run_split_fq.sh" or die;
  print OUTSH "qsub $current_dir/p\$PBS_ARRAYID.split_fq.sh\n";
  close OUTSH;
  push @depend, 'afterokarray:$TWO';
  print ARRAY_SH
    "\$TWO=`qsub -q $qsub_q -t 1-$fq_count $current_dir/run_split_fq.sh`;
echo \$TWO\n
";
...
if [ -d $tempDir ]; then
 ...
 tmp_dir=`mktemp --tmpdir=/tmp -d`
  ...
  print OUTFILE
"for i in \`ls $dir_path/$sample$unpaired_id.f*q\` ; do ln -s \$i \$tmp_dir/$sample.unPaired.fq ; done\n"
    if $paired;
...
"bwa aln -t 8 $bwa_I_switch $genome_path \$tmp_dir/$file$ext > \$tmp_dir/$file.sai";
      ...
      push @aln,
"bwa aln -t 8 -q $bwa_quality $bwa_I_switch $genome_path \$tmp_dir/$file$ext > \$tmp_dir/$file.sai"
        if !$paired;
      push @aln,
"bwa aln -t 8 -q $bwa_quality $bwa_I_switch $genome_path \$tmp_dir/$file$ext > \$tmp_dir/$file.sai"
        if $paired;
...
$count--;
my $depend = join ",", @depend;
print ARRAY_SH
"\$THREE=`qsub -q $qsub_q -W depend=$depend -q $qsub_q -t 0-$count $current_dir/$date.process_raw_reads.sh`
echo \$THREE
";

print ARRAY_SH
"\$FOUR=`qsub -q $qsub_q -W depend=afterokarray:\$THREE $scripts_dir/check_align_sh.pl`
echo \$FOUR
";
...
print ARRAY_SH
"\$FIVE=`qsub -W depend=afterok:\$FOUR -q $qsub_q -t 0-$count $current_dir/$date-do_last.sh`
echo \$FIVE
";
print "
Ok, here we go ...
