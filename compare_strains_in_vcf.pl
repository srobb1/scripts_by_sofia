...
foreach my $loc (sort keys %binary){
  ...
  ## for total shared het
  ## one strain is het, count it
  ## if all strains are a 1, then if it is a 1 add up its total shared hom count;
  if ($hets_count > 0 and $missing_data_count == 0){
        $total_shared_het++;
  }
...
print "\n\ncount of shared HET loci. Ref=T T/T,T/A and Ref=T T/A,T/A are counted:\n";
print "@strains\t$total_shared_het\n";
