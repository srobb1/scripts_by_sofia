...
my $final_out_dir = $out_dir;
mkdir "$final_out_dir", 0777 unless -d "$final_out_dir";
...
##includes each unmapped read
    foreach my $target ( sort keys %targets ) {
        next if $target eq '*';
	$out_dir =`mktemp --tmpdir=/scratch -d`;
	$out_dir =~ s/\n//;
	print "tmpdir=$out_dir\n";
	mkdir "$out_dir/$target", 0777 unless -d "$out_dir/$target";
        my $outfile = "$sample.$target.sam";
        open OUTSAM, ">$out_dir/$target/$outfile" or die $!;
	mkdir "$final_out_dir/$target", 0777 unless -d "$final_out_dir/$target";
...
#print both lft and right read even if only one of the 2 was actullly mapped to this target
	...
	`cp $out_dir/$target/* $final_out_dir/$target/.`; 
	`rm -rf $out_dir`;
    }
}
