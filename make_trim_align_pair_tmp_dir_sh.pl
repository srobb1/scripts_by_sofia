...
foreach my $sample ( sort keys %files ) {
    ...
    foreach my $file ( sort @${ $files{$sample} } ) {
        push @trim_filter,
          "fastq_quality_trimmer -Q$Q -l $minLength -t $minQuality -i $dir_path/$file"
          . ".fq |fastq_quality_filter -Q$Q -q $minQuality -p $minPercent -v -o \$tmp_dir/$file.trimmed.filtered.fq";
        push @aln,
"bwa aln -t 10 $genome_path \$tmp_dir/$file.trimmed.filtered.fq.matched > \$tmp_dir/$file.trimmed.filtered.matched.sai";
    ...
    if ( $pairs == 2 ) {
        ...
        push @clean,
"~/bin/cleanUpPairs_fq.pl \$tmp_dir/$pair1.trimmed.filtered.fq \$tmp_dir/$pair2.trimmed.filtered.fq > \$tmp_dir/$sample.unPaired.fq";
        ##after cleaning a file of unPaired reads is generated
        ##run bwa aln on this file
        ##and bwa samse
        push @clean,
"bwa aln -t 10 $genome_path \$tmp_dir/$sample.unPaired.fq > \$tmp_dir/$sample.unPaired.sai";
        push @clean,
"bwa samse  $genome_path \$tmp_dir/$sample.unPaired.sai \$tmp_dir/$sample.unPaired.fq   > \$tmp_dir/$sample.unPaired.sam";
        push @sam,
"bwa sampe -a $insertLength $genome_path \$tmp_dir/$pair1.trimmed.filtered.matched.sai \$tmp_dir/$pair2.trimmed.filtered.matched.sai \$tmp_dir/$pair1.trimmed.filtered.fq.matched \$tmp_dir/$pair2.trimmed.filtered.fq.matched  > \$tmp_dir/$sample.sam";
    }
    elsif ( $pairs == 1 ) {
        ( $pair1, $pair2 ) = sort @${ $files{$sample} };
        push @sam,
"bwa samse $genome_path \$tmp_dir/$pair1.trimmed.filtered.sai \$tmp_dir/$pair1.trimmed.filtered.fq   > \$tmp_dir/$sample.sam";
    }
    else {
        warn
"error: $sample has $pairs. This sample should have 2 pairs or just 1.\n";
    ...
    #print OUTFILE "cp  \$tmp_dir/* $dir_path.\n";
    #print OUTFILE "rm -rf \$tmp_dir\n";
    print OUTFILE 'remote_host=`hostname`' , "\n";
    print OUTFILE "echo \"scp \$remote_host:\$tmp_dir/* $dir_path/.\" >> $dir_path/toMove.sh\n";
    print OUTFILE "echo \"ssh \$remote_host rm -rf \$tmp_dir\" >> $dir_path/toMove.sh\n";
