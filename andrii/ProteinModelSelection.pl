#!/usr/bin/perl
push @{$ENV{'PATH'}}, '/home/jstajich/bin';
...
if($#ARGV == 1)
  {
    ...
    $partition =  $ARGV[1];
    $cmd = "raxmlHPC-PTHREADS -T 6 -f s -m PROTCATJTT -s ".$alignmentName." -q ".$partition." -n SPLIT_".$alignmentName." \> SPLIT_".$alignmentName."_out";
    system($cmd);
    $count = 0;
...
else
  {
    ...
    $cmd = "raxmlHPC-PTHREADS -T 6 -y -m PROTCATJTT -s ".$alignmentName." -n ST_".$alignmentName." \> ST_".$alignmentName."_out";
    system($cmd);
	...
	$cmd = "raxmlHPC-PTHREADS -T 6 -f e -m ".$aa." -s ".$alignmentName." -t RAxML_parsimonyTree.ST_".$alignmentName." -n ".$AA_Models[$i]."_".$alignmentName."_EVAL \> ".$AA_Models[$i]."_".$alignmentName."_EVAL.out\n";  
	#print($cmd);
	system($cmd);
      }
